...
public class MainScript : MonoBehaviour {
	...
	public void Get(){
		...
		RestClient.GetArray<Post>(basePath + "/posts").Then(res => {
			this.LogMessage("Posts", Proyecto26.JsonHelper.ArrayToJsonString<Post>(res, true));
			return RestClient.GetArray<Todo>(basePath + "/todos");
		}).Then(res => {
			this.LogMessage("Todos", Proyecto26.JsonHelper.ArrayToJsonString<Todo>(res, true));
			return RestClient.GetArray<Points>(basePath + "/users");
		}).Then(res => {
			this.LogMessage("Users", Proyecto26.JsonHelper.ArrayToJsonString<Points>(res, true));
